...
export const findOrderById = async (id) => {
  ...
  const dpUser = order.pickup_dp_id ? await User.findById(order.pickup_dp_id) : null;
  const dpDetail = order.pickup_dp_id ? await DpDetail.findOne({ user_id: order.pickup_dp_id }) : null;
...
export const findLatestOrderRequest = async (order_id) => {
  ...
  const dp = orderRequest.accepted_by ? await User.findById(orderRequest.accepted_by) : null;
