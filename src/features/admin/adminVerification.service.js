...
export const verifyCredentials = async (currentUser, email, password) => {
  if (!currentUser || currentUser.email !== email) {
    throw new Error(
      "The entered email does not match the currently logged-in admin.",
    );
  }
  ...
  if (email === adminEmail && password === adminPassword) {
    ...
    const user = await User.findOne({ email, role: ROLES.ADMIN }).select(
      "+password",
    );
...
export const sendOtp = async (credentialsToken, phone, actionType, amount) => {
  ...
  const adminUser = await adminRepository.findUserByEmailAndType(
    ...
    const otpToken = jwt.sign(
      {
        ...
        action_type: type,
        amount: Number(amt),
        action: "wallet_recharge_otp_sent",
      },
      JWT_SECRET,
      { expiresIn: "5m" },
    );
